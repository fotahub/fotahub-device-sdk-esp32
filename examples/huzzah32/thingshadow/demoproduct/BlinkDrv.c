...
/* 
 * The TIMER_0 of TIMER_GROUP_0 should not be used for blinking led
 * because it's used by the ESP32 FotaHubLib
 */
...
void blink_init(uint16_t blinkPeriod)
{
  ...
  gpio_pad_select_gpio(BLINK_PIN_NUMBER);
  ...
  toggleCounterValue = ((uint64_t)(BlinkDrv_ms_to_micros_any(((int64_t) blinkPeriod)))) >> 1;
}
